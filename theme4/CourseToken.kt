...
import com.example.hi.R.string.need_token
import com.example.hi.ui.theme.CourseRollerApp
...
fun TokenLayout(modifier: Modifier=Modifier.fillMaxSize()) {
    ...
    var token=colculateToken(amount)
...
private fun colculateToken(amount:Double):String{
    return (amount * 1.05).toString()
}
@Composable
fun EditNumberField(
    value: String,
    ...
    TextField(
        ...
        modifier=modifier,
    label={
        Text(stringResource(need_token))
    },
      singLeLine=true,
      keyboardOptions= KeyboardOptions(KeyboardType = KeyboardType.Nunber),
)
}
